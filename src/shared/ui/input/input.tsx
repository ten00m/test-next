...
export const Input: FC<InputProps> = ({ label, className = '', ...props }) => {
    ...
    return (
        <div className="flex flex-col gap-1 relative pt-2.5">
            {label && <label className="absolute h-5 w-10 text-center text-5 text-sm font-medium text-text-secondary bg-background top-0 left-2.5" htmlFor={id}>{label}</label>}
            <input
